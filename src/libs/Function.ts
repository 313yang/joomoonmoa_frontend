...
export const onClickRefresh = async (event?: any) => {
...
export const setIsAutoLogin = (val: boolean) => {
	localStorage.setItem("isAutoLogin", val.toString());
};
...
/** ISO 형식 날짜를 YYYY.MM.DD 형식으로 변환합니다. */
export const formatDateToDotFormat = (isoString: Date) => {
...
export const openToWindow = (url: string, id: string) => {
	return window.open(url, id, "target=_blank");
};